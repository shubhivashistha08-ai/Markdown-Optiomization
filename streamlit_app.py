...
"""
    ...
    with col2:
        ...
        baseline_revenue = (
            filtered_df["Original_Price"].sum()
            * filtered_df["Historical_Sales"].mean()
        )
...
"""
    ...
    with col2:
        ...
        stage_revenue_df = (
            markdown_df.groupby("Stage")["Revenue"].sum().reset_index()
        )
        fig = px.bar(
            stage_revenue_df,
            x="Stage",
...
with tab2:
    ...
    with col1:
        revenue_by_stage = (
            markdown_df.groupby(["Stage", "Category"])["Revenue"]
            .sum()
            .reset_index()
        )
...
""",
    ...
    with col1:
        category_revenue = (
            best_per_product.groupby("Category")["Revenue"]
            .sum()
            .reset_index()
        )
    ...
    with col2:
        ...
        fig.update_layout(
            height=400, yaxis_title="Discount (%)", showlegend=False
        )
        st.plotly_chart(fig, use_container_width=True)
    ...
    season_stage = (
        markdown_df.groupby(["Season", "Stage"])["Revenue"]
        .sum()
        .reset_index()
    )
...
"""
    ...
    top_products["Optimal Discount %"] = (
        top_products["Markdown"] * 100
    ).round(1)
    ...
    st.dataframe(
        ...
        column_config={
            "Expected Revenue": st.column_config.NumberColumn(
                "Expected Revenue", format="$%.0f"
            ),
            "Optimal Discount %": st.column_config.NumberColumn(
                "Optimal Discount", format="%.1f%%"
            ),
        },
    )
...
"""
    ...
    discount_analysis = markdown_df.groupby(
        ["Discount_Range", "Category"]
    ).agg({"Revenue": "sum", "Sales": "sum", "Product_ID": "count"}).reset_index()
    ...
    fig.update_layout(
        height=400, xaxis_title="Discount Range", yaxis_title="Total Revenue"
    )
    st.plotly_chart(fig, use_container_width=True)
    ...
    brand_performance = best_per_product.groupby("Brand").agg(
        {"Revenue": "sum", "Markdown": "mean", "Product_ID": "count"}
    ).reset_index()

    brand_performance["Avg_Discount"] = (
        brand_performance["Markdown"] * 100
    ).round(1)
    ...
    with col2:
        fig = px.scatter(
            ...
            labels={
                "Avg_Discount": "Average Optimal Discount (%)",
                "Product_ID": "Products",
            },
...
st.caption(
    f"📊 Analyzing {len(filtered_df)} products across {len(selected_categories)} categories"
)
